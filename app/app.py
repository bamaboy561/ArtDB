    ...
    with st.container(border=True):
        ...
        if current_user["role"] == "salon":
            ...
            if work_mode == "Новая выгрузка":
                ...
                st.caption("Ниже справа будет открыт отдельный блок для загрузки файла.")
        else:
            work_mode = st.radio(
                "Режим сети",
...
with control_col:
    with st.container(border=True):
        active_screen = render_screen_switcher(
            ...
            description="Вся рабочая навигация собрана здесь, а приложение считает только открытый раздел.",
        )
    ...
    with control_col:
        with st.container(border=True):
            active_analytics_screen = render_screen_switcher(
                ...
                description="Внутри аналитики тоже открыт только один блок за раз.",
            )
            if active_analytics_screen == "Расширенный":
                active_advanced_screen = render_screen_switcher(
                    ...
                    description="Выберите именно тот тип анализа, который нужен сейчас.",
                )
